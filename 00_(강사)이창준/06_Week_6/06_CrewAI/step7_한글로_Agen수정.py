...
def stock_news(ticker):
    ...
    뉴스들은 보기 좋게 정리되어야 합니다. 가능하면 표로 정리하는 것이 좋습니다.
    가져오지 못할 경우, 'None'을 반환합니다.
    """
...
researcher=Agent(
    role="연구원",
    goal="""
        주식에 대한 정보와 뉴스를 종합적으로 파악하여 해석하는 데에 능숙합니다.
        """,
    backstory="""
    ...
    output_file="1_researcher_Agent.md"
)
...
technical_analyst = Agent(
    ...
    output_file="2_technical_Agent.md"
)

financial_analyst = Agent(
    role="금융 분석가",
    goal="""
        금융 제무제표, 내부자 거래 데이터 및 기타 지표를 사용하여 주식의 재무 건전성과 성과를 평가합니다.
        답변은 한글로 작성되어야 합니다.
        tool에서 반환되는 값이 'None'인 경우, 'None'을 반환해야 합니다.
        """,
    backstory="""
        당신은 회사의 재무 상태, 시장 상황 및 질적 데이터를 고려하여 정보화된 권고를 제공하는 매우 경험 많은 투자 자문가입니다.
        """,
    tools=[
        income_stmt,
        balance_sheet,
    ...
    output_file="3_financial_Agent.md"
)
...
hedge_fund_manager = Agent(
                ...
                goal="""
                    ...
                    주식을 사야 할지, 팔아야 할지, 아니면 보유해야 할지에 대한 상세한 투자 권고를 반드시 포함해야 합니다.
                    다른 동료들이 제공한 정보를 기반으로 보고서를 세부적으로 작성해야 합니다.
                    """,
                backstory="""
                    ...
                    """,
                verbose=True,
                output_file="4_hedge_fund_manager_Agent.md",
            )
...
research = Task(
                ...
                agent=researcher,
                expected_output="""
                    최종 답변은 해당 주식에 대하여 조사한 뉴스를 모두 리포트 하고 시장 상황에 대한 자세한 요약이어야 합니다.
                    지금까지 수집한 데이터를 모두 포함하여 답변을 작성하세요.
                    'output_file'은 레포트 형식으로 조사한 모든 데이터를 포함하여 자세히 작성하세요.
                    """,
...
financial_analysis = Task(
                ...
                expected_output="""
                    ...
                    'None'이 반환된 경우, 'Pandas'를 사용하지 말고, 다음단계로 넘어가세요.
...
investment_recommendation = Task(
                ...
                expected_output="""
                    ...
                    'None'이 반환된 경우, 'Pandas'를 사용하지 마세요.
                    답변은 동료들이 제시한 사실에 근거하여 자세히 작성되어야 합니다.
                    """,
