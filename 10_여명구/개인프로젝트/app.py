...
with st.sidebar:
    ...
    error_message = st.text_area("에러 메시지", placeholder="여기에 에러 메시지를 입력하세요...")
    error_code = st.text_area("코드", placeholder="여기에 코드를 입력하세요...")
...
# 사용자 입력 처리 및 OpenAI 호출
if submit_button:
    if not OPENAI_API_KEY:
        st.warning("OpenAI API Key를 설정해주세요.")
    elif not error_message.strip() and not error_code.strip():
        st.warning("에러 메시지나 코드를 입력해주세요.")
    else:
        # 사용자 입력을 결합하여 LLM으로 전달할 메시지 생성
        user_input = f"""
